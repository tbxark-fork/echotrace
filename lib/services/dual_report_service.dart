...
class DualReportService {
  ...
  Future<Map<String, dynamic>> generateDualReport({
    ...
    // 获取好友显示名
    final displayNames = await _databaseService.getDisplayNames([friendUsername]);
    final friendDisplayName = displayNames[friendUsername] ?? friendUsername;
    
    // 1. 亲密度日历
    final intimacyCalendar = await _advancedAnalyticsService.generateIntimacyCalendar(friendUsername);
    
    // 2. 对话天平
    final conversationBalance = await _advancedAnalyticsService.analyzeConversationBalance(friendUsername);
    
    // 3. 最长连聊记录
    final longestStreakRaw = await _advancedAnalyticsService.findLongestStreak(friendUsername);
    ...
    // 5. 第一次聊天时间
    final allMessages = await _analyticsService.getAllMessagesForSession(friendUsername);
    DateTime? firstChatTime;
    DateTime? lastChatTime;
    if (allMessages.isNotEmpty) {
      // 按时间排序
      final sorted = List.from(allMessages)..sort((a, b) => a.createTime.compareTo(b.createTime));
      firstChatTime = DateTime.fromMillisecondsSinceEpoch(sorted.first.createTime * 1000);
      lastChatTime = DateTime.fromMillisecondsSinceEpoch(sorted.last.createTime * 1000);
    }
  ...
  Future<List<Map<String, dynamic>>> getRecommendedFriends({
    ...
    final privateSessions = sessions.where((s) => 
      !s.isGroup && 
    ...
    // 按消息数排序
    friendStats.sort((a, b) => (b['messageCount'] as int).compareTo(a['messageCount'] as int));
