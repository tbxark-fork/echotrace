...
class ImageDecryptService {
  ...
  Uint8List decryptDatV4(String inputPath, int xorKey, Uint8List aesKey) {
    ...
    final result = Uint8List(unpaddedData.length + rawData.length + xoredData.length);
    result.setRange(0, unpaddedData.length, unpaddedData);
    result.setRange(unpaddedData.length, unpaddedData.length + rawData.length, rawData);
    result.setRange(unpaddedData.length + rawData.length, result.length, xoredData);
  ...
  /// [xorKey] XOR 密钥
  /// [aesKey] AES 密钥（仅V4需要）
  Future<void> decryptDatAutoAsync(String inputPath, String outputPath, int xorKey, Uint8List? aesKey) async {
  ...
  /// [xorKey] XOR 密钥
  /// [aesKey] AES 密钥（仅V4需要）
  void decryptDatAuto(String inputPath, String outputPath, int xorKey, Uint8List? aesKey) {
    final version = getDatVersion(inputPath);
