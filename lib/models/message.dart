...
class Message {
  ...
  factory Message.fromMap(Map<String, dynamic> map, {String? myWxid}) {
    ...
    // 步骤2：根据localType解析内容
    final parsedContent = _parseMessageContent(actualContent, localType, messageContent, myWxid);
  ...
  /// 检查解析结果，防止XML泄露
  static String _checkParseResult(String result, int localType, String debugContext) {
  ...
  /// 统一的消息内容解析方法
  static String _parseMessageContent(String content, int localType, String originalMessageContent, String? myWxid) {
    // 调试输出 - 特别关注74614
    final isTargetMessage = kDebugMode && (localType == 244813135921 || content.length > 10000);
    if (isTargetMessage) {
    }
    ...
    // 检查解码后的内容是否包含XML
    final hasXmlContent = decodedContent.contains('<') && decodedContent.contains('>') && 
                         (decodedContent.contains('<msg>') || decodedContent.contains('<appmsg') || decodedContent.contains('<?xml'));
    ...
    switch (localType) {
      case 1: // 文本消息
        final textContent = decodedContent.isNotEmpty ? decodedContent : originalMessageContent;
      ...
      case 244813135921: // 引用消息
        final result = _parseQuoteMessage(decodedContent, originalMessageContent);
    ...
    if (decodedContent.isNotEmpty) {
      ...
      final result = cleanContent.isNotEmpty ? cleanContent : '[未知消息类型($localType)]';
  ...
  static String _extractDurationFromXml(String xml, String attributeName) {
    ...
    try {
      final pattern = RegExp('$attributeName["\s]*=["\s]*(\\d+)', caseSensitive: false);
  ...
  static String _parseRevokeMessage(String xml, String? myWxid) {
    try {
      return XmlMessageParser.parseRevokeMessage(xml, myWxid, null, null) ?? '[撤回消息]';
    } catch (e) {
      return '[撤回消息]';
    }
  }
  ...
  static String? _extractImageMd5(String xml) {
    try {
      final pattern = RegExp(r'md5\s*=\s*["\047]([a-fA-F0-9]+)["\047]', caseSensitive: false);
      final match = pattern.firstMatch(xml);
      return match?.group(1);
    } catch (e) {
      return null;
  ...
  static String _decodeBinaryContent(Uint8List data) {
    ...
    try {
      // 检查是否是zstd压缩数据
      if (data.length >= 4) {
        final magic = (data[3] << 24) | (data[2] << 16) | (data[1] << 8) | data[0];
  ...
  static String _cleanInvalidUtf16(String input) {
    ...
    try {
      ...
      return input.replaceAll(RegExp(r'[^\u0020-\u007E\u4E00-\u9FFF\u3000-\u303F]'), '');
    }
  }

  /// 兼容多种可能字段名读取 is_send
  static int? _readIsSend(Map<String, dynamic> map) {
    for (final key in ['is_send', 'isSend', 'is_sender', 'is_sender_', 'is_send_', 'computed_is_send']) {
  ...
  String get typeDescription {
    switch (localType) {
      ...
      case 244813135921: return '引用消息';
      case 17179869233: return '卡片式链接';
      case 21474836529: return '图文消息';
      case 154618822705: return '小程序分享';
      case 12884901937: return '音乐卡片';
      case 8594229559345: return '红包卡片';
      case 81604378673: return '聊天记录合并转发';
      case 266287972401: return '拍一拍消息';
      case 8589934592049: return '转账卡片';
      case 270582939697: return '视频号直播卡片';
      case 25769803825: return '文件消息';
      default: return '未知消息类型($localType)';
    }
  }
